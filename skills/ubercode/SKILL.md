...
Treat Fable (the newest `claude-fable-*`, reached via the `ask_fable` MCP server) as a smarter reasoning partner for the hard 5% of your work. **Claude Opus 5** is the same tool at roughly half the price and noticeably faster (`ask` with `oracle="opus"`), and several other oracles — **MiniMax-M3**, DeepSeek, GLM, Gemini, GPT-5.6 Sol, Grok, Kimi — are available through the same server for a cross-model check. These oracles have NO tools: they cannot read files, run commands, or browse. They reason ONLY from your question text + the `context` you paste. Weak context in, weak answer out — so the burden is on YOU to package the problem well. Then you verify the reasoning; you never blindly trust it. The 5% framing is the whole point: if a step is routine, cheap to check yourself, or low-stakes, do NOT ask.

## The tools (all on the `ask_fable` MCP server)

- **`ask(question, context="", context_ref=None, session="default", reset=false, oracle="fable")`** — multi-turn, Fable by default. Your default oracle. Reuse the same `session` key for follow-ups (the model keeps the transcript server-side). Pass `context_ref` to inject large context without repasting.
- **`ask(oracle="opus", ...)`** — the same multi-turn tool on Claude Opus: about half Fable's price and faster, same OAuth session, sessions namespaced separately. Prefer it for high-volume or long back-and-forth work; keep Fable for the hardest calls.
- **`ask_model(provider=…)`** — one stateless model, single-turn. `provider` selects the backend: `minimax` (alias `m3`), `deepseek`, `glm` (cheap direct APIs — prefer these), `sonnet`, `gemini`/`codex`/`grok`/`kimi` (local CLIs), and the gateways `ollama`/`lmstudio`/`atlas`/`openrouter`/`ali` (pass `model`). Use for a fast independent take, or when you specifically want the other model's angle.
...
- **Context Bus**: `context(op="pack", key=…, paths=["path:START-END", ...])` — name repo files and the server reads them into a bundle for you, so you point instead of pasting; plus `context(op="write"/"delete", key, value)` for arbitrary blobs and `context_read(key)` (omit `key` to list). All ask tools accept `context_ref` to use these keys instead of repasting `context`.
- **`reset_session(session, save=true)`** — dump-and-clear a Fable thread's transcript.
- **`ask_fable_help(topic)`** — the server's own manual, free and instant (no model call). Harnesses truncate the MCP standing instructions at ~2 KB, so the detail lives in this tool: what to do with a refusal, the context bus, configuring councils, and the full tool menu with every model token. Call it with no argument rather than guessing — every response lists the topics it accepts.
...
**Which tool for the mode:** default to `ask` (Fable). Reach for **`ask_council`** when the stakes justify independent reasoners in parallel — the highest-consequence mode-B reviews and the gnarliest mode-A mysteries (it will explicitly flag where the two disagree). Reach for **`ask_chain`** for an ordered refinement pipeline (e.g. get a draft from MiniMax and have Fable ruthlessly critique and finalize it). Use **`ask_model(provider="minimax")`** when you just want MiniMax's separate opinion cheaply. Note all tools except `ask` are single-turn — for a drill-down conversation, stay on `ask` with a session.
...
```
ask_model(provider="minimax", question="Is there a cleaner way to express this retry-with-backoff than the nested try/except below? Any correctness bug?", context="<paste the function>")
```
...
- `ask` / `ask_model` ok → `{"status":"ok","model":...,"answer":...}` (ask also echoes `session`). Use the reasoning as INPUT, then verify (reproduce, test, re-read the code). For `ask`, continue the thread on the same `session` key. (You might also see `needs_context` if Fable requests a file, or `context_ref_resolved`/`context_ref_missing`.)
- `ask_council` ok → `{"status":"ok","mode":"council","synthesizer":"<model id>","answer":<merged>,"consensus":"strong|partial|divergent|unknown","quorum":"<n>/<requested>","degraded":true|false,"sources":{...}}`. Read `answer`; check `quorum`, `degraded` and `consensus` — `consensus` is a STRING, so `if consensus:` is true even for `divergent`. Consult `sources` for each raw take. A `partial` list (top level or on a source) names models whose answer was CUT OFF at the output cap: the merged answer rests on incomplete input, so re-ask with a narrower question before relying on it.
...
- `{"status":"error","kind":...,"detail":...}` → `timeout`: the turn may still have cost model time, so do NOT blindly retry — narrow the question or trim context first, then re-ask (default 120s/turn, tunable via `ASK_FABLE_TIMEOUT`). `binary_missing`/`sdk_unavailable`/`sdk_error`: a bridge/CLI is unavailable — for `ask_model(provider="minimax")` that's the `mmx` CLI; fall back to your own reasoning or to `ask`. Don't block on it. The rest name what to change rather than something to retry: `context_too_large` (prompt past a CLI's ~120 KB argv limit — use a gateway or trim), `budget_exhausted` (the model spent its whole output budget reasoning — raise the effort/cap or narrow the ask), `payment_required` (out of credits), `disabled` (the operator turned that backend off), `bad_input` (the prompt could not be passed to a CLI at all), `transport_incapable` (that transport cannot carry a follow-up — reset the session). None of these count against the circuit breaker.
...
- Live progress (guard result, each model asked, elapsed time, reasoning excerpts, synthesis) streams to the server's STDERR — visible in the MCP host's server logs (`claude --debug` in Claude Code; the MCP server log in Kimi Code). Silence with `ASK_FABLE_QUIET=1`.
- **Host-side MCP timeout trap (Kimi Code):** the host's MCP request timeout (~60s) can fire BEFORE the server answers — oracle calls routinely run 90–300s. A `Request timed out` MCP error is a client transport timeout, NOT a refusal or a failed call. Verify with `trace_list` / `trace_get` (look for `incomplete`/`cancelled` vs a real answer) before re-asking, and prefer the cheap oracles (`ask_model(provider="minimax")`, `ask_model(provider="glm")`) for quick checks.
