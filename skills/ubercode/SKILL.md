...
- **Host-side MCP timeout trap (Kimi Code):** the host's MCP request timeout (~60s) can fire BEFORE the server answers — oracle calls routinely run 90–300s. A `Request timed out` MCP error is a client transport timeout, NOT a refusal or a failed call. Verify with `trace_list` / `trace_get` (look for `incomplete`/`cancelled` vs a real answer) before re-asking, and prefer the cheap oracles (`ask_m3`, `ask_glm`) for quick checks.
