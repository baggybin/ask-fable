...
- **`context(op="pack", key=…, paths=["path:START-END", ...])`** — let the server read real files into a bundle and pass its key as `context_ref` instead of pasting them into the brief.
- Word choice: phrase questions in ordinary engineering language rather than security jargon.
...
Read the relevant files, docs, and recent commits. Distill a compact **landscape brief**: the core problem, the surrounding system, the hard constraints (perf/cost/compat/security), and the known unknowns. `context(op="write", …)` it under a run key (e.g. `brainstorm-<slug>`) so every oracle turn can `context_ref` it instead of re-pasting.
...
- **Keep the brief on the bus.** One `context(op="write", …)`, many `context_ref` — don't re-paste the growing brief into every council turn.
