...
Save this brief to the server using **`context(op="write", …)`** so you can reference it across multiple brainstorming turns using `context_ref`.
...
```
*(Note: Escalate to a 4-model council or chain ONLY when the architectural decision is foundational and irreversible. Default to Fable+MiniMax for standard ideation. `opus` is the cheapest way to add a second strong Anthropic voice — and `ask(oracle="opus")` is the multi-turn tool for the same model when you just want one quick take; `glm`/`gemini`/`codex`/`grok`/`kimi` are available too when you want more provider diversity.)*
...
- **Point, don't paste:** `context(op="pack", key=…, paths=["src/app.py:1-120", ...])` has the server read the real files into a bundle; pass its key as `context_ref`.
