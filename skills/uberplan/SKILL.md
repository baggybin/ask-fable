...
- Companion: `ubercode` documents general `ask_fable` usage (oracle escalation + second opinion, and the `ask` / `ask_model` / `ask_council` tools). uberplan is the PLANNING specialization; reuse ubercode's ground rules about the oracles.
...
- The `ask_fable` MCP exposes several ask tools; all return ONE JSON object. The oracles have NO tools — they reason ONLY from `question` + `context`. They cannot read files, run commands, or browse. YOU must paste the load-bearing code/structure/constraints into `context`.
  ...
  - **`context(op="pack", key=…, paths=["path:START-END", ...])`** — have the server read the real files into a bundle and pass its key as `context_ref`, instead of pasting the brief by hand.
  ...
  - **`ask(oracle="opus")`** — `ask` on Claude Opus: half the price, faster, its own session namespace. Good for the bulk judging turns when you have many candidates.
  - **`ask_model(provider=…)`** — single-turn, single-model (`minimax`/`deepseek`/`glm` are the cheap direct APIs; `gemini`/`codex`/`grok`/`kimi` the local CLIs; `ollama`/`atlas`/`openrouter`/`ali`/`lmstudio` the gateways). A cheap independent second opinion on one plan (prefer the direct APIs `minimax`/`deepseek`/`glm` over pricier cloud models).
...
- Multi-turn (`ask` only): reuse the same `session` key for follow-ups. Fable keeps context server-side — DON'T re-send the brief or the transcript on later turns. `ask_chain`/`ask_model`/`ask_council` are single-turn: each call is independent, so each must carry its own `context` (or `context_ref` to reuse a saved `context(op="write", …)` block without repasting).
...
- Response shapes: `{"status":"ok",...,"answer":"..."}` | council also adds `"mode":"council"`,`"consensus"` (a string: strong/partial/divergent/unknown),`"quorum"`,`"degraded"`,`"synthesizer"`,`"sources"`, and `"partial"` when a panelist's answer was cut off | chain adds `"mode":"chain"`,`"stages"` | `{"status":"refused","stage":"guard"|"model","reason":"..."}` | `{"status":"error","kind":"timeout|sdk_error|binary_missing|context_too_large|budget_exhausted|payment_required|disabled|bad_input","detail":"..."}`. You may also see `needs_context` if Fable requests a file, or `context_ref_resolved`/`context_ref_missing`.
...
- `error/kind=binary_missing` or repeated `sdk_error`: the bridge is unavailable (`binary_missing` on `ask_model(provider="minimax")`/`ask_council` specifically means the `mmx` CLI isn't installed — Fable via `ask` may still work). If Fable itself is down, SKIP judging entirely, present the N local plans plus your own synthesis, and tell the user "judge unavailable — plans un-refereed."
...
- Speed option: generate the N plans in parallel; keep judging in ONE shared `ask` session for comparative scoring (sequential), OR trade comparison for speed by fanning out independent single-turn judgments (`ask_model`/`ask_council` are naturally single-turn for this). Default: shared Fable session. Core steps above stay harness-agnostic so opencode/DeepSeek agents can follow them verbatim.
