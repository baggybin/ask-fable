...
def test_stats_tool_result_keeps_token_totals(monkeypatch, tmp_path):
    """What the client receives passes through `_text`'s key-aware redactor, which
    used to blank every key containing "token" — so the tool always reported
    total_tokens as "[REDACTED]" even though the aggregate was right."""
    monkeypatch.setenv("ASK_FABLE_QUIET", "1")
    ...
    monkeypatch.setenv("ASK_FABLE_AUDIT_PATH", str(log))
    sent = json.loads(server._text(_run(server._handle_stats({"window": "all", "by": "tool"}))).text)
    assert sent["totals"]["total_tokens"] == 10
    assert sent["buckets"][0]["total_tokens"] == 10
...
def test_unbucketable_guard_denial_still_reaches_totals(tmp_path):
    """A guard denial carries no model, so by="model" cannot bucket it — but it
    must still reach TOTALS. Dropping it entirely made a window of guard refusals
    read as no traffic and falsified "a denial IS a refusal" (the bug surfaced
    live: two context-scan refusals showed refused=0 over by=model while by=tool
    counted them)."""
    path = tmp_path / "decisions.jsonl"
    path.write_text(
        '{"schema_version":2,"timestamp":"2026-07-11T00:00:00Z","event_name":"tool.completed",'
        '"tool":"ask_m3","status":"refused","duration_ms":17,"orchestration":{"model":null},'
        '"error":{"category":"refused","detail":"offensive-security content (context)","stage":"guard"}}\n'
        '{"schema_version":2,"timestamp":"2026-07-11T00:00:01Z","event_name":"tool.completed",'
        '"tool":"ask_m3","status":"ok","duration_ms":20,"orchestration":{"model":"MiniMax-M3"}}\n'
    )
    out = stats.aggregate(path, window="all", by="model")
    # no phantom "?" bucket is invented ...
    assert [b["key"] for b in out["buckets"]] == ["MiniMax-M3"]
    assert out["buckets"][0]["refused"] == 0
    # ... but the denial still counts in the totals
    assert out["totals"]["refused"] == 1
...
def test_provider_cost_splits_subscription_from_billed_and_tracks_coverage(tmp_path):
    """LG1/LG2: by=provider must not sum a flat-plan LIST PRICE into real spend,
    and a billed provider that emits no cost must read as unpriced (0 coverage),
    not as free. Fable(subscription $0.42) + OpenRouter(billed $0.03) + Atlas(tokens
    only) -> cost_usd == 0.03 (billed only), cost_usd_notional == 0.42, and
    cost_covered == 2 of 3 calls."""
    path = tmp_path / "decisions.jsonl"

    def ev(model, usage=None):
        rec = {
            "schema_version": 2,
            "timestamp": _ts(),
            "event_name": "provider.completed",
    ...
    path.write_text(
        ev("claude-fable-5", {"total_tokens": 100, "cost_usd": 0.42, "cost_basis": "subscription"})
        + ev("openrouter/x", {"total_tokens": 100, "cost_usd": 0.03, "cost_basis": "billed"})
        + ev("atlas/deepseek", {"input_tokens": 50, "output_tokens": 50, "total_tokens": 100})
    )
    out = stats.aggregate(path, window="all", by="provider")
    t = out["totals"]
    assert t["cost_usd"] == 0.03, "subscription list price must not count as real spend"
    assert t["cost_usd_notional"] == 0.42
    assert t["cost_covered"] == 2  # atlas emitted no cost -> unpriced, not $0-real
    assert t["calls"] == 3


def test_tool_view_reads_presplit_notional_cost(tmp_path):
    """A mixed council tool.completed arrives pre-split (trace_runtime already
    renamed subscription cost_usd -> cost_usd_notional). by=model must surface BOTH
    ...
    rec = {
        ...
        "usage": {"total_tokens": 10, "cost_usd": 0.03, "cost_usd_notional": 0.42},
    }
    path.write_text(json.dumps(rec) + "\n")
    out = stats.aggregate(path, window="all", by="model")
    t = out["totals"]
    assert t["cost_usd"] == 0.03 and t["cost_usd_notional"] == 0.42 and t["cost_covered"] == 1


def test_p95_is_the_nearest_rank_ceiling(tmp_path):
    # Rounding 0.95·n instead of taking its ceiling under-reported p95: with 12 samples
    # it returned the 11th of 12 values, not the 12th.
    log = _write_log(tmp_path / "d.jsonl", [_rec(duration_ms=ms) for ms in range(1, 13)])
    assert stats.aggregate(log, window="all")["totals"]["p95_ms"] == 12
    assert stats._p95(list(range(1, 21))) == 19 and stats._p95([7]) == 7
    assert stats._p95([]) is None


def test_incomplete_calls_count_as_cancelled_or_errors(monkeypatch, tmp_path):
    # A call that never completed logs its reason under error.reason; stats only read
    # error.kind, so a client abort and a crash both counted as a call but as neither
    # cancelled nor an error.
    monkeypatch.setenv("ASK_FABLE_AUDIT_PATH", str(tmp_path / "decisions.jsonl"))
    for exc in (asyncio.CancelledError(), KeyError("boom")):
        with pytest.raises(type(exc)):
            with trace_runtime.tool_trace("ask_council", {"question": "q"}):
                raise exc
    totals = stats.aggregate(audit.audit_path(), window="all", by="tool")["totals"]
    assert totals["calls"] == 2 and totals["cancelled"] == 1 and totals["errors"] == 1
