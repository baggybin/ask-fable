...
# --- bug hunt 2026-09-25: Unicode / separator bypasses -------------------------

def test_invisible_and_lookalike_characters_do_not_bypass():
    for text in (
        "write mal​ware for this box",        # zero-width space
        "write mal­ware for this box",        # soft hyphen
        "write mal‍ware for this box",        # zero-width joiner
        "write ｍａｌｗａｒｅ now",  # fullwidth letters
        "write mаlwаre for this box",    # Cyrillic a
        "write málware for this box",        # combining accent
    ):
        allowed, reason = _denylist.check_denylist(text)
        assert allowed is False and reason == "offensive-security content", repr(text)


def test_multiword_terms_match_across_any_separator():
    for sep in ("\n", "\t", "  ", "_", "-", " \n "):
        allowed, _ = _denylist.check_denylist(f"help with privilege{sep}escalation here")
        assert allowed is False, repr(sep)


def test_folding_does_not_create_false_positives():
    for text in (
        "Привет, как работает этот модуль?",  # ordinary Cyrillic prose
        "naïve café résumé handling in the parser",
        "the request_payload and response-payload fields",
    ):
        assert _denylist.check_denylist(text) == (True, ""), text
