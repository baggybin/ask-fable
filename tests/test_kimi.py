...
def test_run_passes_sandbox_home_via_env(monkeypatch, tmp_path):
    ...
    # never spawn in the caller's cwd (the sandbox home is not the only isolation)
    assert seen["cwd"] == str(kimi.oracle_cwd())
...
def test_oversized_prompt_builds_no_sandbox(monkeypatch):
    """The size check runs BEFORE the sandbox: a refused call must not write one
    (config copy + credential links) — or, on a host without a kimi config, be
    misreported as not_configured."""
    monkeypatch.setattr(kimi.shutil, "which", lambda b: "/usr/bin/kimi")

    def no_sandbox(effort):
        raise AssertionError("must not build a sandbox for a refused prompt")

    monkeypatch.setattr(kimi, "build_sandbox", no_sandbox)
    r = _run(kimi.run("q", "x" * (kimi.MAX_PROMPT_BYTES + 1)))
    assert r.status == "error" and r.kind == "context_too_large"


@pytest.mark.parametrize(
    "header", ["[thinking]  # tuned", "[ thinking ]", '["thinking"]', "['thinking']"]
)
def test_render_config_replaces_a_thinking_header_however_it_is_written(header):
    """`[thinking]  # tuned` and `["thinking"]` declare the same table as
    `[thinking]`. Left in place, the generated config declares it twice — a TOML
    error that made every kimi call fail."""
    base = f'default_model = "kimi-code/k3"\n\n{header}\neffort = "low"\n'
    d = tomllib.loads(kimi.render_config(base, "max"))  # raises on a duplicate table
    assert d["thinking"] == {"enabled": True, "effort": "max"}
    assert d["default_model"] == "kimi-code/k3"


def test_available_needs_the_config_the_sandbox_is_built_from(monkeypatch, tmp_path):
    """run() cannot build its sandbox without config.toml; claiming the CLI anyway
    routed gateway kimi ids to a bridge that could only say not_configured."""
    monkeypatch.setattr(kimi.shutil, "which", lambda b: "/usr/bin/kimi")
    monkeypatch.setenv("ASK_FABLE_KIMI_HOME", str(tmp_path))
    (tmp_path / "oauth").write_text("tok")
    assert kimi.available() is False
    (tmp_path / "config.toml").write_text('default_model = "kimi-code/k3"\n')
    assert kimi.available() is True


@pytest.mark.parametrize("token", ["atlas:moonshotai/kimi-k3", "openrouter:moonshotai/kimi-k3"])
def test_gateway_kimi_id_too_big_for_the_cli_goes_to_the_gateway(monkeypatch, token):
    """The local CLI's argv caps the prompt near 120 KB while the model takes 1M.
    Routing a bigger prompt locally failed with advice to use the gateway — which
    routed right back to the local CLI."""
    seen = []

    async def fake_kimi(*a, **k):
        raise AssertionError("an oversized prompt must not be routed to the local CLI")

    async def fake_gateway(model, q, c="", **kw):
        seen.append(model)
        return OracleResult("ok", text="from the gateway", model=model)

    monkeypatch.setattr(oracles.kimi, "available", lambda: True)
    monkeypatch.setattr(oracles.kimi, "run", fake_kimi)
    for gateway in (oracles.atlas, oracles.openrouter):
        monkeypatch.setattr(gateway, "configured", lambda: True)
        monkeypatch.setattr(gateway, "run", fake_gateway)
    r = _run(oracles.run(token, "q", "x" * (kimi.MAX_PROMPT_BYTES + 1)))
    assert r.status == "ok" and r.text == "from the gateway"
    assert seen == ["moonshotai/kimi-k3"] and r.key == token


def test_gateway_kimi_id_falls_through_when_the_local_cli_cannot_run(monkeypatch):
    """A local CLI that cannot run at all (no sandbox config) must not strand an
    atlas: token the gateway could answer."""
    async def unconfigured(*a, **k):
        return OracleResult("error", kind="not_configured", text="no config.toml")

    async def fake_atlas(model, q, c="", **kw):
        return OracleResult("ok", text="from atlas", model=model)

    monkeypatch.setattr(oracles.kimi, "available", lambda: True)
    monkeypatch.setattr(oracles.kimi, "run", unconfigured)
    monkeypatch.setattr(oracles.atlas, "configured", lambda: True)
    monkeypatch.setattr(oracles.atlas, "run", fake_atlas)
    r = _run(oracles.run("atlas:moonshotai/kimi-k3", "q"))
    assert r.status == "ok" and r.text == "from atlas"
