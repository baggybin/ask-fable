...
def test_run_max_tokens_stop_is_flagged_truncated(monkeypatch):
    """stop_reason "max_tokens" = the answer was cut off at the cap; flagged, so it
    is never cached as a complete answer."""
    envelope = json.loads(_envelope("The router dispatches by"))
    envelope["stop_reason"] = "max_tokens"
    _patch(monkeypatch, stdout=json.dumps(envelope))
    res = _run(minimax.run("How are handlers registered?"))
    assert res.status == "ok" and res.kind == "truncated"
    assert res.text == "The router dispatches by" and res.meta["partial"] is True
    assert res.telemetry.stop_reason == "max_tokens"
