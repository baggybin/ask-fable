...
def test_backticks_inside_a_json_string_do_not_close_the_block():
    # A ``` inside a JSON string value used to end the fence early: the truncated body
    # failed to parse (sidecar lost) and the rest of the block leaked into the prose.
    text = (
        'Ans.\n\n```json-sidecar\n{"sidecar_version": 1, "recommendation": "apply", '
        '"confidence": "high", "needs_context": ["the ```settings.py``` block"]}\n```'
    )
    prose, sc = sidecar.extract(text)
    assert prose == "Ans."
    assert sc is not None and sc["needs_context"] == ["the ```settings.py``` block"]
    # an empty block is still recognized (and stripped) with the fence on its own line
    assert sidecar.extract("Ans.\n\n```json-sidecar\n```") == ("Ans.", None)
