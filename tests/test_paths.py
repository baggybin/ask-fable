...
def test_ensure_dir_secure_leaves_an_existing_dir_mode_alone(tmp_path):
    # It chmodded ANY existing directory to 0700, including the parent of a user-set
    # path: a 1777 shared dir became 0700 (as root, `--socket /run/x.sock` broke /run).
    shared = tmp_path / "shared"
    shared.mkdir()
    os.chmod(shared, 0o1777)
    assert _paths.ensure_dir_secure(shared) is True
    assert _paths.ensure_dir_secure(shared / "a" / "b") is True
    if sys.platform != "win32":
        assert shared.stat().st_mode & 0o7777 == 0o1777
        # every directory it creates, missing parents included, is still 0700
        assert (shared / "a").stat().st_mode & 0o777 == 0o700
        assert (shared / "a" / "b").stat().st_mode & 0o777 == 0o700


def test_ensure_dir_secure_refuses_a_file_in_the_way(tmp_path):
    blocker = tmp_path / "blocker"
    blocker.write_text("not a directory")
    assert _paths.ensure_dir_secure(blocker) is False
    assert _paths.ensure_dir_secure(blocker / "sub") is False
