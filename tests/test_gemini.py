...
def test_run_ok_and_argv(monkeypatch):
    ...
    # never the caller's repo: agy resolves project context from cwd
    assert seen["kwargs"].get("cwd") == str(isolation.oracle_cwd())


def test_oversized_prompt_is_refused_before_spawning(monkeypatch):
    """`-p` carries the prompt as ONE argv value; above ~128 KiB Popen fails with
    E2BIG, which surfaced as sdk_error and fed the circuit breaker."""
    monkeypatch.setattr(gemini.shutil, "which", lambda b: "/usr/bin/agy")
    ...
    res = _run(gemini.run("Review this module", "x" * 130_000))
    assert res.status == "error" and res.kind == "context_too_large"
    assert "argv" in res.text
...
def test_run_empty_output_is_error(monkeypatch):
    """Empty stdout with rc 0 is agy's denied-tool shape — the stderr tail (which
    names the auto-denied tool and the fix) must ride along, not be dropped."""
    ...
    assert res.text.startswith("Gemini returned no answer")
    assert "nothing came back" in res.text


def test_run_empty_output_without_stderr_stays_bare(monkeypatch):
    _patch(monkeypatch, stdout="   ", stderr="")
    res = _run(gemini.run("Trace dispatch in this module.", timeout=120))
    assert res.status == "error" and res.text == "Gemini returned no answer"
