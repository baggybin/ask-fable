...
from ask_fable import isolation
...
def test_minimax_bridge_spawns_in_the_oracle_cwd(monkeypatch):
    """mmx must not inherit the caller's cwd — the same controlled spawn every
    other CLI bridge uses, so no oracle can read the surrounding repo."""
    monkeypatch.setattr(minimax.shutil, "which", lambda b: "/usr/bin/mmx")
    seen = {}

    async def fake_cli(argv, **kw):
        seen.update(kw)
        return minimax.cli_gate.CliRun(0, '{"text": "hi"}', "", False)

    monkeypatch.setattr(minimax.cli_gate, "run_cli_async", fake_cli)
    r = _run(minimax.run("q"))
    assert r.status == "ok"
    assert seen.get("cwd") == str(isolation.oracle_cwd())
