...
def test_resolve_default_and_dedup_and_order():
    ...
    # an explicit all-unknown list is NOT the default council — it resolves to an
    # empty panel so the caller can refuse it; unknowns are de-duped too
    assert oracles.resolve(["nope"]) == ([], ["nope"])
    assert oracles.resolve(["nope", "NOPE"]) == ([], ["nope"])
...
def test_resolve_fallback_grows_with_deepseek_key(monkeypatch):
    ...
    # only an OMITTED list borrows the default; an all-unknown one never does
    assert oracles.resolve(["nope"]) == ([], ["nope"])
...
def test_resolve_applies_aliases_like_the_chain():
    ...
    assert oracles.resolve(["gpt5"]) == ([], ["gpt5"])
...
def test_resolve_ollama_tokens():
    ...
    # a bare ollama: (no model) is unknown — and, alone, resolves to nothing
    assert oracles.resolve(["ollama:"]) == ([], ["ollama:"])
