...
def test_ollama_council_honors_synthesizer(monkeypatch):
    """ask_council(provider='ollama') used to drop `synthesizer` on the floor — even
    an invalid one ran as Fable. It resolves like every other provider council now:
    a valid token adjudicates, an invalid one is bad_args before any fan-out."""
    _allow(monkeypatch)
    monkeypatch.setenv("ASK_FABLE_OLLAMA_API_KEY", "k")
    keys, synth_keys = [], []
    ...
    async def fake_synthesis(key, prompt, **kw):
        ...
        return OracleResult("ok", key=key, text="CODEX MERGED", model="gpt-5.6-sol")
    ...
    args = {"provider": "ollama", "question": "How does routing work?",
            "models": ["gpt-oss:120b-cloud", "kimi:cloud"]}
    out = _run(server._handle_council({**args, "synthesizer": "gpt"}))
    assert out["status"] == "ok" and out["answer"] == "CODEX MERGED"
    assert synth_keys == ["codex"]
    assert out["synthesis"]["requested"] == "codex" and out["synthesis"]["used"] == "codex"

    keys.clear()
    out = _run(server._handle_council({**args, "synthesizer": "definitely-not-a-model"}))
    assert out["status"] == "error" and out["kind"] == "bad_args"
    assert keys == []  # refused before the fan-out
