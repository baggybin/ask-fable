"""ask_fable ask_deepseek handler — the standalone deepseek-flash tool.
...
def _cfg():
    return ProviderConfig(key="deepseek", label="deepseek-flash", base_url="https://x",
...
def test_ok_returns_answer_and_model(monkeypatch):
    ...
    spy = _stub(monkeypatch, OracleResult("ok", text="DeepSeek says hi", model="deepseek-flash"))
    out = _run(server._handle_deepseek({"question": "How does routing work here?", "context": "def r(): ..."}))
    assert out == {"status": "ok", "model": "deepseek-flash", "answer": "DeepSeek says hi",
...
def test_model_refused(monkeypatch):
    _allow(monkeypatch)
    _configured(monkeypatch)
    _stub(monkeypatch, OracleResult("refused", text="not about code", model="deepseek-flash"))
...
def test_model_error(monkeypatch):
    _allow(monkeypatch)
    _configured(monkeypatch)
    _stub(monkeypatch, OracleResult("error", kind="timeout", text="slow", model="deepseek-flash"))
