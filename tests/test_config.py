...
def test_concurrent_saves_do_not_lose_updates(tmp_path, monkeypatch):
    """S2: two instances' read-modify-write must not clobber each other's untouched
    fields. Three threads each hammer a distinct key; the flock serializes the RMW so
    all three survive (without it, whole-file replace loses the others' last writes)."""
    import threading
    ...
    threads = [threading.Thread(target=writer, args=(k,)) for k in ("alpha", "beta", "gamma")]
    for t in threads:
        t.start()
    for t in threads:
        t.join()
    final = cfg.load()
    assert final.get("alpha") == "alpha24"
    assert final.get("beta") == "beta24"
    assert final.get("gamma") == "gamma24"
