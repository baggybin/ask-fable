...
def test_dump_redacts_secrets(monkeypatch, tmp_path):
    # Every other on-disk sink runs redact_text; the transcript dump must too.
    monkeypatch.setenv("XDG_STATE_HOME", str(tmp_path))
    store = sessions.SessionStore()
    store.record_turn(
        "leaky",
        question="why does password=hunter2hunter2 fail?",
        answer='the config has {"api_key": "AAAABBBBCCCCDDDD"}',
        session_id=None,
        thinking="secret: plain-thinking-secret",
    )
    text = Path(store.reset("leaky", save=True)).read_text()
    for leaked in ("hunter2hunter2", "AAAABBBBCCCCDDDD", "plain-thinking-secret"):
