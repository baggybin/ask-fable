...
from ask_fable import cli_gate, health
from ask_fable.oracle_common import (
    cli_error_detail,
    error_status,
    http_error_detail,
...
def test_http_moderation_403_is_a_provider_refusal_not_auth():
    """OpenRouter's moderation layer answers a flagged prompt with 403. That is the
    provider refusing THIS payload, not a bad key — and a refusal is reported as
    `refused`, which never counts against the breaker."""
    err = ('HTTP 403: {"error":{"code":403,"message":"openai/gpt-5 requires moderation '
           'on OpenAI. Your input was flagged for \\"violence\\"."}}')
    kind, text = http_error_detail(label="OpenRouter", error=err, http_status=403)
    assert kind == "provider_refusal" and error_status(kind) == "refused"
    assert "provider safeguard refusal" in text and "flagged" in text
    # A plain 403 is still a bad key.
    kind, _ = http_error_detail(label="Atlas", error="HTTP 403: forbidden", http_status=403)
    assert kind == "auth_failed" and error_status(kind) == "error"


def test_http_402_is_a_billing_state_not_backend_ill_health():
    """Out of credits is chronic until the operator pays — like a bad key, it must
    not trip the breaker (it used to classify as sdk_error)."""
    err = "HTTP 402: This request requires more credits, or fewer max_tokens."
    kind, text = http_error_detail(label="OpenRouter", error=err, http_status=402)
    assert kind == "payment_required" and "requires more credits" in text
    assert kind in health._NON_HEALTH_KINDS
