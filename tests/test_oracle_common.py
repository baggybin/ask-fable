...
def test_is_provider_refusal_by_stop_reason():
    assert oc.is_provider_refusal("refusal", "") == "provider safeguard refusal"
    assert oc.is_provider_refusal("REFUSAL", "") == "provider safeguard refusal"


def test_is_provider_refusal_by_message_phrase():
    msg = "API Error: Opus 4.8's safeguards flagged this message."
    assert oc.is_provider_refusal(None, msg) == "provider safeguard refusal"
    assert oc.is_provider_refusal(None, "Apply to the Cyber Verification Program.") is not None


def test_is_provider_refusal_negatives():
    assert oc.is_provider_refusal(None, "rate limit reached") is None
    assert oc.is_provider_refusal("end_turn", "all good") is None
    assert oc.is_provider_refusal(None, "") is None
