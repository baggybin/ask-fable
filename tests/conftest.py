...
def _isolate_state_dir(monkeypatch, tmp_path):
    """Point the XDG state dir at tmp so ``isolation.oracle_cwd()`` is a throwaway
    and no test creates ``~/.local/state/ask_fable/oracle-cwd`` or writes there."""
...
@pytest.fixture(autouse=True)
def _no_code_execution(monkeypatch):
    """Keep the sandbox gate OFF unless a test opts in.

    `ask_verify` reads `ASK_FABLE_ALLOW_RUN` on every call, so on a dev box that exports
    it the whole verify suite shells out to real `bwrap` twice per test (0.3s -> 3.4s).
    A gate this consequential must not be decided by ambient environment."""
...
@pytest.fixture(autouse=True)
def _isolate_context_hwm(monkeypatch, tmp_path):
    """Keep the reader-side rollback guard per-test: bus-mode reads/writes in
    tests must never touch the operator's real high-water marks."""
    monkeypatch.setenv("ASK_FABLE_CONTEXT_HWM_PATH", str(tmp_path / "context_hwm.db"))
