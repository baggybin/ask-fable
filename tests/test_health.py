...
def test_breaker_ignores_request_and_billing_states():
    """An output budget spent reasoning (budget_exhausted) and an account out of
    credits (payment_required) say nothing about the backend's health."""
    b = Breaker()
    for _ in range(20):
        b.record("openrouter:v/m", "error", "budget_exhausted")
        b.record("openrouter:v/m", "error", "payment_required")
    assert b.state("openrouter:v/m") == "closed"


def test_breaker_ignores_prompts_too_big_for_a_cli():
    """A prompt too big for a CLI's argv is a property of the request: five
    big-context calls used to open the breaker and shed the backend for everyone."""
    b = Breaker()
    for _ in range(20):
        b.record("grok", "error", "context_too_large")
    assert b.state("grok") == "closed"
