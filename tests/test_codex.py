...
def test_run_ok_and_argv(monkeypatch):
    ...
    # codex reads AGENTS.md from its workspace root (cwd) — never the caller's
...
def test_oversized_prompt_is_refused_before_spawning(monkeypatch):
    ...
    which surfaced as sdk_error — five such calls opened the breaker for 300s."""
    monkeypatch.setattr(codex.shutil, "which", lambda b: "/usr/bin/codex")
    ...
    res = _run(codex.run("Review this module", "x" * 130_000))
    assert res.status == "error" and res.kind == "context_too_large"
    assert "argv" in res.text and "openai/gpt-5.6-sol" in res.text
