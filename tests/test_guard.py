...
# ── context scan (Layer 2b, default ON) ─────────────────────────────────

def _hit_on_bad(prompt: str) -> tuple[bool, str]:
    return (False, "offensive-security content") if "BAD" in (prompt or "") else (True, "")


def test_context_hit_blocks_and_is_labelled(monkeypatch):
    """A clean question with offending framing in `context` is blocked, and the
    reason names `context` so the caller reframes the right field."""
    monkeypatch.setattr(guard, "check_denylist", _hit_on_bad)
    allowed, reason = guard.check("a long enough clean question about the router", context="BAD")
    assert allowed is False
    assert reason == "offensive-security content (context)"


def test_question_hit_is_not_labelled_context(monkeypatch):
    monkeypatch.setattr(guard, "check_denylist", _hit_on_bad)
    allowed, reason = guard.check("a BAD question that is long enough", context="clean")
    assert allowed is False
    assert reason == "offensive-security content"


def test_context_scan_lifted_by_trusted(monkeypatch):
    monkeypatch.setattr(guard, "check_denylist", _hit_on_bad)
    allowed, reason = guard.check("a long enough clean question about the router",
                                  context="BAD", trusted=True)
    assert allowed is True and reason != ""


def test_context_scan_disabled_by_knob(monkeypatch):
    monkeypatch.setenv("ASK_FABLE_GUARD_SCAN_CONTEXT", "0")
    monkeypatch.setattr(guard, "check_denylist", _hit_on_bad)
    allowed, _ = guard.check("a long enough clean question about the router", context="BAD")
    assert allowed is True


def test_context_scan_default_on_when_unset(monkeypatch):
    monkeypatch.delenv("ASK_FABLE_GUARD_SCAN_CONTEXT", raising=False)
    assert guard._scan_context_enabled() is True


def test_bundled_denylist_context_scan_and_allowlist():
    """End-to-end with the REAL denylist: a bare offense term in `context` blocks,
    while the newly-added benign code phrases ("data payload", …) do not."""
    from ask_fable import _denylist

    # bare "payload" outside a benign phrase is an offense term
    assert _denylist.check_denylist("context holds an exfil payload")[0] is False
    # benign engineering phrases rescue it
    for phrase in ("data payload", "form payload", "payload type", "payload field"):
        assert _denylist.check_denylist(f"the {phrase} building code")[0] is True
