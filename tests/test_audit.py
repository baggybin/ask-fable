...
def test_record_noops_while_a_trace_is_active(monkeypatch, tmp_path):
    """S1: on the server every call runs inside tool_trace, so audit.record() returns
    early (the trace's tool.completed supersedes it) — nothing is written to the flat
    log. The isolated record() tests above run with NO active trace; this documents
    the production reality so they aren't read as 'the flat audit log is live'."""
    import ask_fable.trace_runtime as tr
    monkeypatch.setenv("ASK_FABLE_AUDIT_PATH", str(tmp_path / "decisions.jsonl"))
    monkeypatch.setattr(tr, "current", lambda: object())  # simulate an active trace
    audit.record(decision="allowed", stage=None, reason="marker",
                 question="q", context="", session="s", model="m")
    assert not (tmp_path / "decisions.jsonl").exists()  # returned before any write
