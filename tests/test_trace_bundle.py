...
@pytest.mark.parametrize(
    "key",
    [
        "token", "access_token", "refresh_token", "api_token", "auth_token", "bearer_token",
        "id_token", "accessToken", "X-Auth-Token", "csrftoken", "context_bus_token",
    ],
)
def test_credential_token_keys_are_redacted(key):
    assert trace_bundle.redact_value({key: "opaque-credential"}) == {key: "[REDACTED]"}


@pytest.mark.parametrize(
    "key",
    [
        "tokens", "max_tokens", "total_tokens", "input_tokens", "output_tokens",
        "cache_read_input_tokens", "token_count", "totalTokenCount", "maxCompletionTokens",
    ],
)
def test_token_count_keys_are_not_redacted(key):
    """Every tool result runs through redact_value, which blanked any key containing
    "token" — so stats, trace_get and the model listings reported every token COUNT
    as "[REDACTED]"."""
    assert trace_bundle.redact_value({"usage": {key: 1500}}) == {"usage": {key: 1500}}
