...
def test_extract_backticks_inside_a_json_string():
    # A code sample in `evidence` carries ``` inside a JSON string; the unanchored closing
    # fence ended the block there, so the ledger parsed to None and JSON leaked into prose.
    body = ('{"debate_version":1,"role":"propose","claims":[{"id":"C1","claim":"x",'
            '"evidence":"```python\\nx = retry()\\n```"}]}')
    prose, led = dl.extract(_block(body))
    assert prose == "answer prose here"
    assert led is not None and dl.claims(led)[0]["evidence"] == "```python\nx = retry()\n```"
