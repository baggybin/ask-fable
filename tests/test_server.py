...
def test_annotation_classifications_are_honest():
    ...
    assert by_name["list_atlas_models"].readOnlyHint is True
    assert by_name["list_atlas_models"].openWorldHint is True

    # Data-losing local ops are flagged destructive.
    for name in ("context_write", "context_delete", "reset_session"):
