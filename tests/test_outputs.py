...
def test_empty_save_env_is_treated_as_unset(tmp_path, monkeypatch, empty):
    # An empty ASK_FABLE_SAVE used to count as an explicit "on", saving raw
    # questions/answers in safe trace mode where unset saves nothing.
    monkeypatch.setenv("ASK_FABLE_SAVE", empty)
    monkeypatch.setenv("ASK_FABLE_TRACE_MODE", "safe")
    monkeypatch.setenv("ASK_FABLE_OUTPUT_DIR", str(tmp_path))
    assert outputs.save(tool="ask", model="m", question="secret", answer="answer") is None
    monkeypatch.setenv("ASK_FABLE_TRACE_MODE", "full")  # ...and follows trace mode like unset
    assert outputs.save(tool="ask", model="m", question="q", answer="a") is not None
