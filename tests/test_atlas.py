...
def test_cap_and_timeout_override_precedence(monkeypatch):
    from ask_fable import config

    monkeypatch.delenv("ASK_FABLE_ATLAS_MAX_TOKENS", raising=False)
    monkeypatch.delenv("ASK_FABLE_ATLAS_TIMEOUT", raising=False)
    assert at.max_tokens_override() is None
    assert at.timeout_override() is None

    config.save({"atlas_max_tokens": 8192, "atlas_timeout": 240})
    assert at.max_tokens_override() == 8192
    assert at.timeout_override() == 240.0

    # config wins over env
    monkeypatch.setenv("ASK_FABLE_ATLAS_MAX_TOKENS", "2048")
    monkeypatch.setenv("ASK_FABLE_ATLAS_TIMEOUT", "90")
    assert at.max_tokens_override() == 8192
    assert at.timeout_override() == 240.0

    # env is the fallback when the config key is absent
    config.save({"atlas_max_tokens": None, "atlas_timeout": None})
    assert at.max_tokens_override() == 2048
    assert at.timeout_override() == 90.0

    # invalid values disable the override rather than raise
    config.save({"atlas_max_tokens": "nope", "atlas_timeout": "-3"})
    assert at.max_tokens_override() is None
    assert at.timeout_override() is None


def test_run_applies_cap_and_timeout_overrides(monkeypatch):
    from ask_fable import config
    ...
    class _Resp:
        ...
        def read(self):
            return b'{"choices":[{"message":{"content":"ans"}}]}'
    ...
    config.save({"atlas_max_tokens": 8192, "atlas_timeout": 300})
    res = _run(at.run("xai/grok-4.5", "q", effort="standard"))
    assert res.status == "ok"
    # the pin lifts standard's 4096 cap and its 240s wall-clock
    assert seen["body"]["max_tokens"] == 8192
    assert seen["timeout"] == 300.0
    # the global output ceiling still wins over the pin
    monkeypatch.setenv("ASK_FABLE_MAX_TOKENS", "2048")
    res = _run(at.run("xai/grok-4.5", "q", effort="standard"))
    assert res.status == "ok"
...
def test_run_dropped_connection_is_an_error_result_not_a_raise(monkeypatch):
    """A gateway/proxy that closes a long request without a status line raises
    RemoteDisconnected from getresponse(), which urllib does NOT wrap in URLError.
    It must come back as a network error, not escape run() and abort a chain."""
    import http.client
    ...
    monkeypatch.setenv("ASK_FABLE_ATLAS_API_KEY", "k")
    monkeypatch.setattr(at.urllib.request, "urlopen", drop)
    res = _run(at.run("xai/grok-4.5", "q", effort="standard", timeout=1))  # no budget to retry
...
def test_run_length_finish_is_flagged_truncated(monkeypatch):
    """finish_reason "length" = the output cap cut the answer off. It used to come
    back as a clean ok and be cached for an hour as complete."""
    _serve(monkeypatch, {"choices": [{"message": {"content": "The fix is to chan"},
                                      "finish_reason": "length"}]})
    res = _run(at.run("acme/z", "q", effort="quick"))
    assert res.status == "ok" and res.kind == "truncated"
    assert res.text == "The fix is to chan" and res.meta["partial"] is True


def test_run_length_with_no_content_is_budget_exhausted(monkeypatch):
    """A reasoning model that spends the whole `quick` budget thinking returns
    content null — not an sdk_error 'empty response' that opens the breaker."""
    _serve(monkeypatch, {"choices": [{"message": {"content": None},
                                      "finish_reason": "length"}]})
    res = _run(at.run("acme/z", "q", effort="quick"))
    assert res.status == "error" and res.kind == "budget_exhausted"
    assert "acme/z" in res.text


def test_catalog_survives_a_body_cut_short(monkeypatch):
    """The shared catalog GET is 'never raises' too: an IncompleteRead while
    reading the body degrades to cloud_ok False instead of escaping."""
    import http.client
    ...
    class _Resp:
        ...
        def read(self):
            raise http.client.IncompleteRead(b'{"data":[', 5000)
