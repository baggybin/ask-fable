...
def test_strong_requires_full_requested_panel():
    """T2 (bug hunt 2026-09-09): 'strong' needs EVERY requested model to answer and
    agree. 2 of 5 unanimous survivors is 'partial'. panel_size (answered) still
    separates unknown/partial; requested (asked) gates strong."""
    assert server._consensus(["apply", "apply"], panel_size=2, requested=5) == ("partial", False)
    assert server._consensus(["apply"] * 5, panel_size=5, requested=5) == ("strong", False)
    # back-compat: requested defaults to panel_size for standalone callers
    assert server._consensus(["apply", "apply"], panel_size=2) == ("strong", False)


def test_survivor_council_reports_partial_not_strong(monkeypatch):
    """End-to-end: 3 requested, 1 errors, the 2 survivors agree → 'partial', and the
    vote vector shows the missing voter."""
    out, _ = _council_with(monkeypatch, {
        "fable": _oracle("fable", "claude-fable-5", "apply"),
        "minimax": _oracle("minimax", "MiniMax-M3", "apply"),
        "glm": OracleResult("error", key="glm", kind="timeout", text="glm to", model="GLM-5.2"),
    })
    assert out["consensus"] == "partial"
    assert out["consensus_votes"] == {"apply": 2, "no_sidecar": 1}


def test_failed_synthesis_never_claims_safe_to_act():
    # A council whose synthesis failed returns one panelist's raw reply; the advice must
    # say so rather than "full quorum answered and was synthesized — safe to act".
    from ask_fable import oracles

    ok = [
        OracleResult("ok", key="fable", text="a", model="claude-fable-5"),
        OracleResult("ok", key="deepseek", text="b", model="deepseek-v4-pro"),
        OracleResult("ok", key="minimax", text="c", model="MiniMax-M3"),
    ]
    assert oracles.distinct_labs([r.key for r in ok]) >= 2
    out = server._council_envelope(ok, 3, None, consensus="strong")
    nxt = out["recommended_next_action"]
    assert "safe to act" not in nxt and "synthesis failed" in nxt
