...
def test_atlas_listing_fetches_the_catalog_off_the_event_loop(monkeypatch):
    """atlas.catalog is a blocking urllib fetch (6 s timeout). Called directly in the
    async handler it froze every other in-flight tool call for the whole fetch; every
    sibling lister already runs its catalog in a worker thread."""
    import threading
    ...
    def fake_catalog(*args, **kwargs):
        ...
        return {"cloud_ok": False, "featured": [], "menu": [], "models": []}

    monkeypatch.setattr(server.atlas, "catalog", fake_catalog)

    async def go():
        out = await server._handle_list_models({"provider": "atlas"})
        return out, threading.current_thread()

    out, loop_thread = _run(go())
    assert out["cloud_ok"] is False
    assert fetched_on and fetched_on[0] is not loop_thread
