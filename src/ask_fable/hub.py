...
_SESSION_META_DDL = (
    ...
    "PRIMARY KEY (project, session_key, agent_id))"
)


def _ensure_session_meta(conn: sqlite3.Connection) -> None:
    """Create session_meta, migrating a pre-fix table whose PRIMARY KEY omitted
    ``project``. The old ``PRIMARY KEY (session_key, agent_id)`` collided across
    projects on one machine (same default session + agent id, different cwd): the
    project-less upsert flipped the row's ``project`` to whichever wrote last, so the
    other project's dashboard went blank. Just changing the DDL is not enough —
    ``CREATE TABLE IF NOT EXISTS`` is a SILENT no-op on an existing DB, after which the
    new ``ON CONFLICT(project, …)`` raises "does not match any PRIMARY KEY" and the
    best-effort wrapper swallows it, quietly stopping the mirror. So detect the old
    schema by its stored DDL and rebuild. This is a rebuildable cache — ``session_peek``
    reads the append-only ``turns`` table, which is untouched — and the old rows already
    merged two projects' ``turn_count`` under the colliding key, so there is nothing
    correct to copy: drop and recreate empty (the view repopulates on the next write).
    IF EXISTS / IF NOT EXISTS keep two racing processes idempotent; a reader that catches
    the sub-millisecond rebuild window gets one ``store_unavailable`` and re-reads."""
    row = conn.execute(
        "SELECT sql FROM sqlite_master WHERE type='table' AND name='session_meta'"
    ).fetchone()
    if row is not None and "PRIMARY KEY (project, session_key, agent_id)" not in (row[0] or ""):
        conn.execute("DROP TABLE IF EXISTS session_meta")
    conn.execute(_SESSION_META_DDL)
    conn.commit()
...
def write_turn(
    *,
    ...
    if conn is None:
        return
    now = time.time()
    # The hub is a cross-agent, cross-process store read back by session_peek/
    # session_list, so it was the one persistence sink that stored the prompt and the
    # model's answer in cleartext while audit/outputs/trace all redact. Redact here too
    # (a secret in the question, or one Fable echoed from context into the answer).
    question = redact_text(question or "")[0]
    answer = redact_text(answer or "")[0]
    try:
        conn.execute(
        ...
        conn.execute(
            ...
            "ON CONFLICT(project, session_key, agent_id) DO UPDATE SET "
...
def prune_junk(
    *,
    ...
    try:
        ...
        # Matched on the FULL key (project, session_key, agent_id): two projects
        # sharing the default session + agent id are two rows, and must not keep
        # each other's rows alive or share each other's counts and last turn.
        conn.execute(
            "DELETE FROM session_meta AS m WHERE NOT EXISTS "
            "(SELECT 1 FROM turns t WHERE t.project = m.project "
            "AND t.session_key = m.session_key AND t.agent_id = m.agent_id)"
        )
        # Recount + refresh last_* for remaining meta from the latest surviving turn.
        rows = conn.execute(
            "SELECT project, session_key, agent_id FROM session_meta"
        ).fetchall()
        for proj, skey, aid in rows:
            count = conn.execute(
                "SELECT COUNT(*) FROM turns "
                "WHERE project = ? AND session_key = ? AND agent_id = ?",
                (proj, skey, aid),
            ).fetchone()[0]
            latest = conn.execute(
                "SELECT question, answer, oracle, status, sdk_session_id, ts "
                "FROM turns WHERE project = ? AND session_key = ? AND agent_id = ? "
            ...
            conn.execute(
                ...
                "WHERE project = ? AND session_key = ? AND agent_id = ?",
                (count, q, a, orc, st, sdk, ts, proj, skey, aid),
            )
        conn.commit()
...
def _sweep(conn: sqlite3.Connection) -> None:
    ...
    try:
        ...
        if count > cap:
            ...
            # Evict oldest turns, then drop orphaned session_meta rows — matched on the
            # full (project, session_key, agent_id) key, as in prune_junk.
            conn.execute(
            ...
            # S6: turn_count is bumped on every write but eviction removed some turns,
            # so a surviving session's counter over-reported. Recompute it from the
            # turns that actually remain (matches what prune_junk already does), so
            # session_stats no longer inflates counts after a sweep.
            conn.execute(
                "UPDATE session_meta SET turn_count = ("
                "SELECT COUNT(*) FROM turns t WHERE t.project = session_meta.project "
                "AND t.session_key = session_meta.session_key "
                "AND t.agent_id = session_meta.agent_id)"
            )
        conn.commit()
