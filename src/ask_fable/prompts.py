...
SERVER_INSTRUCTIONS = (
    "Strong external reasoning models (Fable, Claude Opus 5, plus councils) — a "
    ...
    "a change spanning >2-3 files -> `ask` (Fable, multi-turn) or `ask_opus5` "
    "(~half the price, faster; prefer it for long back-and-forth).\n"
    ...
    "`ask_m3`/`ask_deepseek`/`ask_glm` (cheap, prefer these); "
...
HELP_TOPICS: dict[str, str] = {
    "refused": (
        ...
        'method: …"\n'
        '3. TOOL OUT OF SCOPE -> the everyday word "payload" (JSON/HTTP body) '
        'trips the guard. Say "request body" / "response body" / "JSON '
        'body" instead.\n\n'
        ...
        "`trusted=true` on the `ask` call. The operator must have authorized this "
        "— the guard still runs but in log-only mode, and the audit records the "
        "trusted-session marker.\n\n"
    ),
    "context": (
        ...
        "point, the server reads, the model sees the bundle. `context_read` reads "
        "a blob back, `context_delete` removes one.\n\n"
        "`context_ref` is accepted by `ask`, `ask_opus5`, every single-model tool, "
    ...
    "setup": (
        ...
        "structured picker fallback otherwise; then call `ask_atlas` with the "
        "user's pick. Atlas models can also join `ask_council`, `ask_chain`, and "
        "`ask_debate` as `atlas:<model-id>` tokens (the catalog endpoint is free). "
        "**OpenRouter** works the same way — `list_openrouter_models(task=…)` "
        "(free) then `ask_openrouter`, or `openrouter:<model-id>` tokens in any "
    ...
    "tools": (
        "## The full tool menu\n\n"
        "CORE REASONING\n"
        "  ask                     Fable, multi-turn. The default move. Reuse "
        "`session` for follow-ups.\n"
        "  ask_opus5               Same tool on Claude Opus 5 — ~half the price, "
        "faster, separate session namespace.\n"
        ...
        "  ask_ollama_council / ask_atlas_council / ask_openrouter_council\n"
        "                          Provider-specific panels; the last is a "
        "cross-lab panel on one key.\n\n"
        "SINGLE MODELS\n"
        "  ask_m3 (MiniMax-M3), ask_deepseek, ask_glm — cheap direct APIs, prefer "
        "these.\n"
        "  ask_gemini, ask_codex (GPT-5.6 Sol), ask_grok, ask_kimi — via local "
        "CLIs.\n"
        "  ask_ollama, ask_atlas, ask_openrouter — gateways; call the matching "
        "`list_*_models` first (free).\n"
        "  Prefer a dedicated tool over a gateway when both reach the same model.\n\n"
        "MODEL TOKENS (usable in any council / chain / debate)\n"
        "  fable, fable51, opus, deepseek, minimax, glm, gemini, codex, grok, "
        "kimi,\n"
        "  ollama:<model>, atlas:<model-id>, openrouter:<model-id>.\n"
        ...
        "it with `bad_args`.\n\n"
        "CONTEXT BUS   context_pack, context_write, context_read, context_list, "
        "context_delete (topic `context`).\n"
        "OPS           stats, trace_list, trace_get, reset_session, "
        "configure_tracing,\n"
        "              configure_ollama_council, configure_atlas_council, "
        "configure_openrouter_council,\n"
        "              session_list, session_peek, session_stats."
    ),
}
