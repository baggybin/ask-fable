...
def _model_efforts(model: str, timeout: float = 6.0) -> tuple[list[str], str | None] | None:
    ...
    Best-effort: a catalog miss must degrade to "send no reasoning_effort", never
    to a failed turn. A lookup can download the whole ~400-model catalog, so it
    BLOCKS (``run`` calls it off the event loop) and its answers are kept: a hit
    for the process; a model the catalog does not list (a ``:nitro`` variant, an
    alias) and a failed download until the next refresh, so neither re-downloads
    the catalog on every call."""
    global _effort_refresh_at
    with _EFFORT_LOCK:
        if model in _EFFORT_CACHE:
            return _EFFORT_CACHE[model]
        if time.monotonic() < _effort_refresh_at:
            return None  # fetched (or failed) recently, and this id was not in it
        cat = catalog(timeout)
        ok = bool(cat.get("cloud_ok"))
        _effort_refresh_at = time.monotonic() + (_EFFORT_REFRESH_S if ok else _EFFORT_RETRY_S)
        if ok:
            for it in cat["menu"]:
                _EFFORT_CACHE[it["model_id"]] = (it["supported_efforts"], it["default_effort"])
        return _EFFORT_CACHE.get(model)


_EFFORT_CACHE: dict[str, tuple[list[str], str | None]] = {}
# One catalog download at a time (a council fans several members out at once), and
# none before `_effort_refresh_at` (monotonic): after a good download an id it did
# not list stays a miss for _EFFORT_REFRESH_S; a failed one is retried after
# _EFFORT_RETRY_S.
_EFFORT_LOCK = threading.Lock()
_EFFORT_REFRESH_S = 600.0
_EFFORT_RETRY_S = 60.0
_effort_refresh_at = 0.0
...
async def run(
    ...
    *,
    ...
    # The lookup may download the catalog (a blocking GET, up to 6s): run it off
    # the event loop, or every other in-flight call stalls behind it.
    reasoning_effort = await asyncio.to_thread(
        _clamp_effort, model, preset.get("reasoning_effort")
    )
    metadata: dict = {}
    ...
    if err:
        ...
        return OracleResult(
            error_status(kind), kind=kind, text=text, model=model,
    ...
    shaped = shape_stopped(text, _finish_reason(metadata), label=model, thinking=thinking)
    return OracleResult(
        shaped.status,
        text=shaped.text,
        kind=shaped.kind,
