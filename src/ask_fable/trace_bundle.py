...
# Whether a key NAMES a credential is decided by the same word-by-word rule the
# text redactor uses (`redaction.is_secret_key`), so the two agree: `access_token`
# and `csrftoken` are credentials, `max_tokens` and `token_count` are counts, and
# `tokenizer` is neither. The old substring regex here disagreed in both
# directions — it spared every plural (`api_tokens`, `access_tokens`) and blanked
# `tokenizer`, `input_token_limit` and `secretary`.
# String values under any key still pass through redact_text's credential patterns.
