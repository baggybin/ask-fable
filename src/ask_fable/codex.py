...
async def run(
    ...
    *,
    ...
    too_large = argv_too_large(
        message, binary=CODEX_BINARY,
        advice=(
            "GPT is reachable over HTTP instead (ask_openrouter or ask_atlas with "
            "'openai/gpt-5.6-sol'), or trim `context`."
        ),
    )
    if too_large:
        return OracleResult(
    ...
    try:
        ...
        try:
            cap = await cli_gate.run_cli_async(
                argv, gate="codex", timeout=timeout, cwd=str(oracle_cwd())
            )
