...
# The closing fence must start its own line (`^` under MULTILINE), so a ``` inside a JSON
# string value — never at a line start, JSON strings can't hold a raw newline — can't end
# the block early and leak the rest of it into the prose.
