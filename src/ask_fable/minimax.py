    ...
    *,
    ...
    try:
        ...
        run = await cli_gate.run_cli_async(
            argv, gate="mmx", timeout=timeout, input_text=messages, cwd=str(oracle_cwd())
        )
    ...
    # Honor the shared REFUSED contract, and flag an answer the output cap cut off.
    shaped = shape_stopped(text, stop_reason, label=model, thinking=thinking)
    shaped.model = model
