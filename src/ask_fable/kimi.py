...
# the HTTP route (the cap every argv-prompt CLI shares, see oracle_common).
MAX_PROMPT_BYTES = MAX_ARGV_PROMPT_BYTES
...
def available() -> bool:
    """True when the CLI is installed AND can run a turn: its login material
    exists (an unauthenticated CLI would sit at a device-code prompt we cannot
    answer) and so does the ``config.toml`` every turn's sandbox is built from —
    without it ``run`` can only report ``not_configured``, so claiming the CLI
    here would route gateway kimi ids to a bridge that cannot answer them."""
    if shutil.which(KIMI_BINARY) is None:
        return False
    home = real_home()
    if not (home / "config.toml").is_file():
        return False
    return (home / "oauth").exists() or (home / "credentials").exists()


def prompt_fits(question: str, context: str = "") -> bool:
    """True when this transport can carry the prompt at all (see
    :data:`MAX_PROMPT_BYTES`) — what decides whether a gateway kimi id may be
    served by the local CLI instead of the gateway's own full context."""
...
# Group 1 is the name as written, between the brackets.
_TABLE_HEADER_RE = re.compile(r"^\[\[?([^\[\]]+)\]\]?\s*(?:#.*)?$")


def _table_name(raw: str) -> str:
    """A header's table name, normalized: ``[thinking]``, ``[ thinking ]`` and
    ``["thinking"]`` all declare the same table, so they must compare equal.
    Quotes are stripped per dotted segment, and only the outer whitespace — the
    space inside a quoted key like ``[" thinking "]`` is part of that key."""
...
def _strip_table(text: str, name: str) -> str:
    ...
    for line in text.splitlines():
        ...
        if header:
            # Compare the NAME, not the raw line: `[thinking]  # tuned` or
            # `["thinking"]` declares the same table, and one left in place is a
            # duplicate [thinking] that makes every generated config unloadable.
            skipping = _table_name(header.group(1)) == name
        if not skipping:
            out.append(line)
    return "\n".join(out)
...
def render_config(base: str, effort: str) -> str:
    ...
    """
    # `thinking` is stripped as a TABLE and as a top-level BARE KEY: we emit our own
    # [thinking] table, and an operator config carrying `thinking = true` (or an inline
    # `thinking = { ... }`) made the merged file a "Cannot overwrite a value" parse
    # error, so every kimi call died on an opaque CLI config error — the same failure
    # the `[thinking]  # tuned` fix closed for the table spelling (L20).
    body = _strip_bare_key(_strip_table(base, "thinking"), "default_permission_mode")
    body = _strip_bare_key(body, "thinking")
    return (
...
async def run(
    ...
    *,
    ...
    # Sized BEFORE the sandbox is built: an oversized prompt is refused whatever
    # the sandbox's state, so it must neither write a sandbox for a call that
    # never spawns nor be misreported as `not_configured` on a host without one.
    message = compose(question, context)
    too_large = argv_too_large(
        message, binary=KIMI_BINARY,
        advice=(
            "The model's 1M context is reachable over HTTP instead: ask_atlas with "
            "'moonshotai/kimi-k3', or trim `context`."
        ),
    )
    if too_large:
        return OracleResult(
            "error", kind="context_too_large", text=too_large,
