...
def _scan_context_enabled() -> bool:
    """Whether Layer 2 also runs the denylist over ``context``.

    ``ASK_FABLE_GUARD_SCAN_CONTEXT`` — **default ON** when unset. The provider's
    own safeguard reads the WHOLE payload, so a question that passes on its own
    can still get the call refused upstream when the framing rides in ``context``;
    scanning it here moves that block to OUR deterministic, logged, free gate.
    Set the knob to a falsey value (``0``/``false``/``no``/``off``) to disable.

    NOTE the default is ON when unset — the opposite of the server's ``_flag()``
    helper (unset → off), so this reads ``config.setting`` directly rather than
    reusing it. Config file overrides env, so it is runtime-togglable."""
    raw = (config.setting("ASK_FABLE_GUARD_SCAN_CONTEXT") or "").strip().lower()
    if not raw:
        return True
...
def check(question: str, context: str = "", *, trusted: bool = False) -> tuple[bool, str]:
    ...
    # Layer 2b — the SAME denylist over `context` (default on; see
    # `_scan_context_enabled`). The provider's safeguard reads the whole payload,
    # so framing parked in `context` can get a call refused upstream even when the
    # question alone is clean -- and that refusal is opaque, unlogged and
    # non-deterministic. Catching it here keeps the gate deterministic and moves
    # the block to ours. `trusted` lifts it, exactly as it does for the question.
    if _scan_context_enabled():
        allowed_ctx, label_ctx = check_denylist(context)
        if not allowed_ctx:
            if trusted:
                return True, label_ctx or "prohibited content (context) (trusted)"
            return False, f"{label_ctx or 'prohibited content'} (context)"
