...
def _enabled() -> bool:
    # Empty counts as unset (an `ASK_FABLE_SAVE=` line in an MCP registration), not
    # as an explicit "on".
    explicit = (os.environ.get("ASK_FABLE_SAVE") or "").strip()
    if explicit:
        return explicit.lower() not in ("0", "false", "no", "off")
    # config-over-env (matches trace_bundle + ToolTrace.mode): a runtime trace_mode
    # toggle via configure_tracing governs answer-saving too, not just the env default.
    return (config.setting("ASK_FABLE_TRACE_MODE") or "safe").strip().lower() == "full"
