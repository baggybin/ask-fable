...
# How run_cli reports arguments that could not reach a process at all (a NUL byte or
# an unpaired surrogate in argv). cli_error_detail keys on it: bad input, not a sick CLI.
ARGV_REJECTED_PREFIX = "cannot pass the arguments to "
...
def _drain(stream, cap: int, sink: dict) -> None:
    """Read ``stream`` to EOF, keeping only the first ``cap`` chars — the rest is counted and
    discarded so the child never blocks on a full pipe and the parent's memory stays bounded."""
    kept: list[str] = []
    total = 0
    try:
        while True:
...
def _communicate_capped(
    proc: subprocess.Popen, input_text: str | None, timeout: float, cap: int
) -> CliRun:
    """Like ``proc.communicate(timeout=…)`` but bounds retained output to ``cap`` chars per
    stream, so a runaway printer can't balloon the parent's memory before the kill. Keeps the
    same never-orphan/never-escape contract: on timeout or a mid-run failure the whole group
    is SIGKILLed and reaped. Readers run in daemon threads so a grandchild holding a pipe open
    can never wedge the caller."""
    out_sink: dict = {"text": ""}
    err_sink: dict = {"text": ""}
    t_out = threading.Thread(target=_drain, args=(proc.stdout, cap, out_sink), daemon=True)
    t_err = threading.Thread(target=_drain, args=(proc.stderr, cap, err_sink), daemon=True)
    t_out.start()
    t_err.start()
    ...
    try:
        ...
        try:
            ...
            pass
        return CliRun(None, "", f"failed mid-run: {type(exc).__name__}: {exc}", timed_out=False)
    t_out.join(timeout=5)
    t_err.join(timeout=5)
    return CliRun(None if timed_out else proc.returncode, out_sink["text"], err_sink["text"], timed_out)
    ...
    try:
        with hold(gate, timeout=timeout):
            ...
            try:
                ...
                # The arguments cannot reach a process at all: a NUL byte ("embedded
                # null byte") or an unencodable surrogate (UnicodeEncodeError is a
                # ValueError) in a prompt that rides argv. That is the caller's
                # input, not a broken CLI — say so rather than escape the same way.
                return CliRun(
                    None, "",
                    f"{ARGV_REJECTED_PREFIX}{argv[0]}: {exc} — remove NUL bytes "
                    "and unpaired surrogates from the question/context",
                    timed_out=False,
                )
            ...
            if proc_handle is not None and not proc_handle.set(proc):
                # set() SIGKILLed it; reap it here rather than leaving a zombie until
                # CPython's next _active sweep, and close the pipes we opened.
                with contextlib.suppress(subprocess.TimeoutExpired, OSError, ValueError):
                    proc.communicate(timeout=5)
                return CliRun(None, "", "", timed_out=False)
            # Opt-in bounded reader (used by the run: sandbox): cap retained output as it
            # streams. Default None keeps the exact communicate() path every other bridge uses.
            if max_output_bytes is not None:
                return _communicate_capped(proc, input_text, remaining, max_output_bytes)
            try:
