...
def record(
    *,
    ...
    try:
        ...
        # On the server EVERY tool call runs inside tool_trace, so current() is always
        # set and this returns early: the trace's richer tool.completed event (written
        # to this same decisions.jsonl, and what stats reads) supersedes the flat record
        # below. This path is therefore the STANDALONE / no-active-trace fallback only.
        # NOTE: fields unique to this record (trusted_session, ASK_FABLE_AUDIT_RAW raw
        # capture) are consequently NOT captured on the server — quorum/consensus/
        # synth_fallback still are, via set_orchestration on the trace.
