...
def _pinned_number(key: str, env_var: str) -> float | None:
    """Config ``key`` (JSON number or numeric string) then env ``env_var`` as a
    positive float, or None. A present-but-invalid config value disables the
    override rather than falling through to the env (same precedence as
    :func:`default_effort`)."""
    raw: object = config.load().get(key)
    if raw is None:
...
def max_tokens_override() -> int | None:
    """Operator-pinned Atlas output cap (config ``atlas_max_tokens`` /
    ``ASK_FABLE_ATLAS_MAX_TOKENS``); ``None`` when unset or invalid.

    Replaces the effort preset's ``max_tokens`` (up or down) while staying under
    the global output ceiling. The lever for Atlas's gateway, which returns HTTP
    504 for a request still running at ~242s: ~8192 fits the window where
    ``deep``'s 16384 may not.
    """
    value = _pinned_number("atlas_max_tokens", "ASK_FABLE_ATLAS_MAX_TOKENS")
    return int(value) if value is not None else None


def timeout_override() -> float | None:
    """Operator-pinned Atlas wall-clock seconds (config ``atlas_timeout`` /
    ``ASK_FABLE_ATLAS_TIMEOUT``); ``None`` when unset or invalid. Replaces the
    effort preset's timeout without touching the token cap."""
    return _pinned_number("atlas_timeout", "ASK_FABLE_ATLAS_TIMEOUT")
...
async def run(
    ...
    *,
    ...
    Config ``atlas_max_tokens`` / ``atlas_timeout`` (env
    ``ASK_FABLE_ATLAS_MAX_TOKENS`` / ``ASK_FABLE_ATLAS_TIMEOUT``) replace the
    effort preset's cap and wall-clock; the cap stays under the global ceiling.
    """
    preset = effort_preset(effort)
    timeout = timeout if timeout is not None else (timeout_override() or preset["timeout"])
    key = api_key()
    if not configured():
        return OracleResult(
            "error",
    ...
    pinned_cap = max_tokens_override()
    cap = min(pinned_cap if pinned_cap is not None else preset["max_tokens"], _max_tokens())
    metadata: dict = {}
    retry_count = 0
    final_http_status: int | None = None
    dropped = False  # the last POST lost its connection mid-exchange (retryable)
    ...
    def _post(with_reasoning_effort: bool) -> tuple[str | None, dict, str | None, int | None]:
        ...
        try:
            ...
            dropped = True
            return None, {}, extract_http_error(e), None
    ...
    try:
        (text, _thinking, err), status_retries = await call_with_transient_retry(
            ...
            # connection — the reasoning_effort feature retry inside _call can
            # stack on top.
            retryable=lambda r: bool(r[2]) and (
                final_http_status in RETRYABLE_HTTP_STATUSES or dropped
            ),
        )
    ...
    shaped = shape_stopped(text, _finish_reason(metadata), label=model)
