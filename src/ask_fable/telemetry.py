...
class TraceEvent:
    ...
    # A content digest of a per-source answer (sha256/chars/bytes/redaction_count),
    # written by Trace.provider. Additive and optional, so it stays schema v2:
    # older events omit it, older readers ignore it.
    output: EventBlock | None = None
    ...
    def new(
        ...
        **fields: str | float | EventBlock | None,
        ...
        return cls(
            ...
            output=_block_field(fields, "output"),
        )
...
def _block_field(
    fields: Mapping[str, str | float | EventBlock | None], name: str
) -> EventBlock | None:
    value = fields.get(name)
