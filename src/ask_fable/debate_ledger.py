...
# The closing fence must start its own line (`^` under MULTILINE): a ``` inside a JSON
# string value (a code sample in `evidence`) no longer ends the block early — JSON strings
# can't hold a raw newline, so it is never at a line start. Unanchored, it cut the body to
# unparseable JSON and leaked the rest of the block into the prose.
