...
def _get_json(url: str, timeout: float) -> dict | None:
    ...
    try:
        ...
        urllib.error.URLError, *DROPPED_CONNECTION_ERRORS, ValueError, UnicodeDecodeError,
        TimeoutError,
    ):
        return None
    ...
    *,
    ...
    try:
        (text, thinking, err), status_retries = await call_with_transient_retry(
            ...
            # connection — the think-flag feature retry inside _call can stack on top.
    ...
    shaped = shape_stopped(text, metadata.get("done_reason"), label=model, thinking=thinking)
    usage = (
        usage_if_available(
            ProviderUsage(
