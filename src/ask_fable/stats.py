...
def _p95(sorted_ms: list[int]) -> int | None:
    """Nearest-rank p95 over an ascending list, or None when empty. The rank is
    ceil(0.95·n): rounding it instead read the 11th of 12 values, under-reporting."""
    if not sorted_ms:
        return None
...
def _finalize(key: str, b: dict) -> dict:
    ...
    return {
        ...
        "cost_usd_notional": round(b["cost_usd_notional"], 6),
        "cost_covered": b["cost_covered"],
...
def aggregate(
    ...
    *,
    ...
    those too. A call that never completed counts as ``cancelled`` when the client
    aborted it and as an error when it crashed."""
    ...
    for rec in _iter_records(path):
        ...
        if rec.get("schema_version") == 2:
            ...
            if by == "provider" and isinstance(rec.get("provider"), dict):
                if rec["provider"].get("transport") == "cache":
                    continue
                # M10: a panel member cancelled while still QUEUED (`run_bounded`'s
                # "never got a slot" branch) records a 0 ms provider.completed. It
                # never reached a backend — flagged here, counted as cancelled only
                # below.
                queued_skip = rec["provider"].get("transport") == "skipped"
            rec = dict(rec)
            if queued_skip:
                rec["_queued_skip"] = True
        ...
        # A record this view can't bucket (a guard denial carries no model; a
        # non-cache record has no cache status) is dropped from the BREAKDOWN but
        # must still reach TOTALS — same treatment as circuit_open below. Dropping
        # it entirely made the default by="model" report a window of guard refusals
        # as no traffic at all, and made the promise that a denial counts under
        # ``refused`` false.
        unbucketed = (by == "cache" and key not in {"hit", "miss"}) or (
            by == "model" and key == "?"
        )
        targets = (totals,) if unbucketed else (buckets.setdefault(key, _empty_bucket()), totals)
        error = rec.get("error")
        ...
        # A call that never completed (tool_trace's `incomplete` terminal event) says why
        # under error.reason, not error.kind: a client abort is a cancellation, a crash
        # (`exception` / `no_complete`) an error. It used to count as a call but as neither.
        incomplete = decision == "incomplete"
        cancelled = isinstance(error, dict) and (
            error.get("kind") == "cancelled"
            or (incomplete and error.get("reason") == "cancelled")
        )
        dur = rec.get("duration_ms")
        if rec.get("_queued_skip"):
            # Never reached a backend: cancelled, but not a call and not a latency
            # sample. Counting it as both inflated `calls` and pulled `avg_ms` down
            # (one real 30 s call + five queued cancels reported avg 5 s).
            for tgt in targets:
                tgt["cancelled"] += 1
                _count_cache(tgt, rec)
            continue
        for tgt in targets:
            ...
            if decision == "allowed":
                ...
                tgt["cancelled"] += 1
            elif decision == "error" or incomplete:
                tgt["errors"] += 1
            ...
            if isinstance(usage, dict):
                ...
                # Cost is basis-aware. A flat-plan (subscription) oracle reports a
                # LIST PRICE, not money spent; summing it into cost_usd next to a
                # billed gateway's real charge yields a number that is neither.
                # Keep them apart: cost_usd = real billed spend, cost_usd_notional =
                # subscription list-price reference. tool.completed already arrives
                # split (trace_runtime renames subscription cost_usd →
                # cost_usd_notional); provider.completed carries the raw pair, so we
                # branch on cost_basis here too. cost_covered = how many calls carried
                # ANY known cost, so a $0 at 0 coverage reads as "unpriced" (Atlas /
                # DeepSeek emit tokens but no cost) rather than "free".
                cost = usage.get("cost_usd")
                notional = usage.get("cost_usd_notional")
                covered = False
                if isinstance(cost, (int, float)):
                    if usage.get("cost_basis") == "subscription":
                        tgt["cost_usd_notional"] += float(cost)
                    else:
                        tgt["cost_usd"] += float(cost)
                    covered = True
                if isinstance(notional, (int, float)):
                    tgt["cost_usd_notional"] += float(notional)
                    covered = True
                if covered:
                    tgt["cost_covered"] += 1
