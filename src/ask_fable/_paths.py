...
def ensure_dir_secure(d: Path) -> bool:
    ...
    raises. Each directory it creates gets its mode via ``os.chmod`` after
    ``mkdir`` — on filesystems that don't support POSIX perms the chmod fails
    silently and the directory is left at the process umask.

    An EXISTING directory is left exactly as it is: ``d`` is often the parent of
    a user-set path (``ASK_FABLE_CONTEXT_PATH``, ``context-busd --socket``), which
    can be a shared or system directory — chmodding ``/tmp`` or ``/run`` to 0700
    breaks every other user of it."""
    missing: list[Path] = []
    p = d
    ...
    try:
        for p in reversed(missing):
            try:
                # Creating at 0o700 closes the create-at-umask-then-chmod window (mkdir
                # applies it atomically); the chmod corrects an umask that strips owner bits.
                os.mkdir(p, 0o700)
