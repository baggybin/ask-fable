...
def _guard_refusal(reason: str) -> dict:
    return {
        ...
        "how_to_reframe": (
            ...
            "again. REFRAME instead: ask the underlying engineering question "
            "about a named symbol, with the code in `context`. "
...
_TOOL_ANNOTATIONS: dict[str, types.ToolAnnotations] = {
    ...
    "ask_opus5": _model_call("Ask Claude Opus 5"),
    "ask_m3": _model_call("Ask MiniMax-M3"),
    "ask_glm": _model_call("Ask GLM"),
    "ask_deepseek": _model_call("Ask DeepSeek"),
    "ask_gemini": _model_call("Ask Gemini"),
    "ask_codex": _model_call("Ask GPT-5.6 Sol (Codex)"),
    "ask_grok": _model_call("Ask Grok"),
    "ask_kimi": _model_call("Ask Kimi"),
    ...
    "ask_ollama": _model_call("Ask an Ollama Cloud model"),
    "ask_ollama_council": _model_call("Ask an Ollama Cloud council"),
    "ask_atlas": _model_call("Ask an Atlas Cloud model"),
    "ask_atlas_council": _model_call("Ask an Atlas Cloud council"),
    "ask_openrouter": _model_call("Ask an OpenRouter model"),
    "ask_openrouter_council": _model_call("Ask an OpenRouter council"),
    ...
    "list_openrouter_models": _reads_remote("List OpenRouter models"),
    "list_atlas_models": _reads_remote("List Atlas Cloud models"),
    "list_ollama_models": _reads_remote("List Ollama Cloud models"),
    # config writers — persist settings to the local config file
    "configure_openrouter_council": _writes_config("Configure OpenRouter council"),
    "configure_ollama_council": _writes_config("Configure Ollama council"),
    "configure_atlas_council": _writes_config("Configure Atlas council"),
    "configure_tracing": _writes_config("Configure tracing"),
    # context store writes — overwrite a blob by key (shared keyspace, no undo)
    "context_write": _mutates_store("Write a context blob"),
    "context_pack": _mutates_store("Pack repo files into context"),
    # pure local reads
    "context_read": _reads_local("Read a context blob"),
    "context_list": _reads_local("List the context store"),
    ...
    "context_delete": _mutates_store("Delete a context blob"),
    "reset_session": _mutates_store("Reset a Fable session"),
}
...
async def _handle_claude_ask(
    ...
    *,
    ...
    key = session_prefix + str(args.get("session") or "default")
...
async def _handle_conference(args: dict) -> dict:
    ...
    if len(selected) < 2:
        return {
            ...
            "detail": (
                ...
                "resolved — configure keys or pass `models`."
            ),
    ...
    outcome = await conference.run_conference(
        topic, list(selected), context=context, rounds=rounds, synthesizer=synthesizer
    )
    ...
    *,
    ...
    def finish(
        ...
        *,
        ...
        # Neither a stalemate nor a failed adjudication produced a NEUTRAL verdict, so
        # neither may carry the proposer's own (often "high") confidence.
        if resolution in ("stalemate", "adjudicator_unavailable") and sc:
...
async def _council(
    ...
    *,
    ...
    consensus, material = _consensus(recs, len(ok), confs, requested=len(selected))
    votes = _vote_vector(recs, len(selected))
...
def _resolve_context(context: str, context_ref) -> tuple[str, list[str], list[str], list[str]]:
    ...
    model answered blind. ``last_error()`` is set by the failing op and cleared on any
    clean connect, so read immediately after each get it tells the two apart per key:
