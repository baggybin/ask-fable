...
# v4: the oracle layer keys on the oracle token (gateway included) and the resolved
# default effort — a v3 entry could hand one gateway's answer to the same model id
# on another (atlas:X vs openrouter:X), or a demoted `fable` answer to `fable51`.
_KEY_VERSION = 5  # v5: the synthesis turn now sees the CODE CONTEXT (answers change)
    ...
    canon = json.dumps(
        {
            ...
            "q": q,
            # surrogatepass: a lone surrogate (valid in an MCP JSON string) must hash,
            # not raise UnicodeEncodeError out of the whole tool call.
            "c": hashlib.sha256((context or "").encode("utf-8", "surrogatepass")).hexdigest(),
