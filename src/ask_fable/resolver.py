...
- **Subset-aware dedupe:** a range spec subsumed by a whole-file spec of the same
  file is dropped once that file is admitted (its content is then present); if the
  file is not admitted, the range is tried in its place. Exact-duplicate specs
  collapse. Overlapping-but-distinct ranges are kept (merging is a deferred decision).
...
def _dedupe(
    items: list[tuple[int, Resolved, str]],
) -> tuple[list[tuple[int, Resolved, str]], dict[str, list[tuple[int, Resolved, str]]]]:
    """Collapse exact duplicates (keeping the first) and hold back range specs
    subsumed by a whole-file spec of the same file. Returns ``(kept, held)`` with
    ``held`` keyed by file: a held range is dropped silently only if its whole file
    is admitted (the content is then present) — otherwise ``pack`` tries it in the
    file's place, so it is never lost from both ``included`` and ``skipped``."""
    whole = {r.abs_path for _, r, _ in items if r.start is None}
    seen: set[tuple[str, int | None, int | None]] = set()
    out: list[tuple[int, Resolved, str]] = []
    held: dict[str, list[tuple[int, Resolved, str]]] = {}
    for idx, r, spec in items:
        ...
        if r.start is not None and r.abs_path in whole:
            held.setdefault(r.abs_path, []).append((idx, r, spec))
        else:
...
def pack(
    ...
    *,
    ...
    def admit(idx: int, r: Resolved, spec: str) -> Reason | None:
        """Add ``spec``'s block to the bundle, or return why it can't go in."""
        nonlocal total
        if stopped:
            return Reason.BUDGET_EXHAUSTED
        if len(admitted) >= max_files:
            return Reason.MAX_FILES
    ...
    for idx, r, spec in order:
        reason = admit(idx, r, spec)
        if reason is None:
            continue
        skipped.append({"spec": spec, "reason": reason.value})
        # A whole file that is not admitted hands off to the ranges it subsumed: each
        # is tried in its place — even past the file's own overflow — so it lands in
        # `included` or `skipped` instead of silently in neither.
        for sub_idx, sub_r, sub_spec in held.get(r.abs_path, []):
            sub_reason = admit(sub_idx, sub_r, sub_spec)
            if sub_reason is not None:
                skipped.append({"spec": sub_spec, "reason": sub_reason.value})
                stopped = stopped or sub_reason is Reason.BUDGET_EXHAUSTED
        stopped = stopped or reason is Reason.BUDGET_EXHAUSTED
