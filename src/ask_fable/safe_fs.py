...
# Bytes read at a time while skipping a line before a range's START. A minified
# bundle or data dump can be one multi-hundred-MB line; it must never be held whole.
_SKIP_CHUNK = 1 << 20
...
def _is_blocked(rel_path: str, extra: tuple[str, ...]) -> bool:
    ...
    Every comparison is casefolded: on a case-insensitive filesystem (APFS, NTFS)
    ``.ENV`` or ``.GIT/config`` opens the real secret file, and ``realpath`` keeps
    the case the caller typed rather than the on-disk case."""
    parts = tuple(p.casefold() for p in Path(rel_path).parts)
    ...
    if base == ".env" or base.startswith(".env."):
        return True
    for tok in extra:
        tok = tok.casefold() if tok else tok
...
def read_content(resolved: Resolved, *, max_bytes: int) -> tuple[str, int] | Reason:
    ...
    and never read past the cap (a file can grow, or understate its size, after the
    ``fstat``). Range: read incrementally and stop after ``end``; the cap applies to
    the bytes actually COLLECTED, so a small slice of a huge file succeeds. Memory
    stays bounded by the cap either way: lines before ``start`` are skipped in
    chunks and a collected line is read no further than the cap, so one huge line
    is never held whole. A NUL byte in the collected bytes marks it ``BINARY``;
    otherwise decode UTF-8 with ``errors=replace`` so a stray odd byte never raises."""
    try:
        with open(resolved.abs_path, "rb") as fh:
            ...
            # Range: count "\n"-terminated lines, collect [start, end], stop at end.
            collected = bytearray()
            lineno = 0
            while lineno < resolved.end:
                keep = lineno + 1 >= resolved.start
                started = False
                while True:  # one line, in bounded pieces
                    piece = fh.readline(max_bytes + 1 - len(collected) if keep else _SKIP_CHUNK)
                    if not piece:
                        break
                    started = True
                    if keep:
