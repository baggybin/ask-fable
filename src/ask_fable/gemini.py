...
TOOL EXPOSURE — ``agy`` is an *agent* CLI whose print mode runs under a permission
policy it cannot prompt for, and that policy is not ours to set. Probed on the
installed build: ``search_web`` works unconditionally; ``read_url`` (page fetch) is
**auto-denied** unless the operator has added an allow-rule to agy's own
``settings.json`` (``permissions.allow``, e.g. ``read_url(*)``); and a denial does
not degrade gracefully — the turn aborts and agy prints NOTHING on stdout with
rc 0, which is why an empty answer here now carries agy's stderr back to the caller.

There is no ``--allowed-tools`` / ``--disallowed-tools`` / ``--permission-mode``
flag (this CLI defines none of them, verified), and ``--sandbox`` restricts only
the terminal, not tool classes. So ``ask_gemini``'s "pure reasoning" default is
enforced by the *prompt*, not a tool gate — a plain print turn was also observed
creating a file. ``web_search=True`` (used by ``ask_websearch``) swaps in the
research/OSINT prompt plus a search-only note (:data:`ask_fable.prompts.
WEBSEARCH_SYSTEM_PROMPT_AGY`) that steers the model off ``read_url``, since one
attempted fetch would otherwise cost the whole answer.

Single-turn: no session to resume.
"""
...
async def run(
    ...
    *,
    ...
    ``web_search`` (opt-in, used by ``ask_websearch``) marks the turn as a
    live-web research turn and ``system_prompt`` overrides the oracle scope
    contract (``ask_websearch`` passes a research/OSINT prompt). Neither is a
    tool gate: ``agy`` exposes ``search_web``/``read_url_content`` unconditionally
    and has no allow/deny flag, so this is a prompt-level switch (see the module
    docstring). The one behavioural change is a longer ``--print-timeout`` — a
    search turn spends extra round-trips that a pure-reasoning turn does not."""
    ...
    message = f"{system_prompt or ORACLE_SYSTEM_PROMPT}\n\n{compose(question, context)}"
    too_large = argv_too_large(
        message, binary=GEMINI_BINARY,
        advice=(
            "Gemini is reachable over HTTP instead (a 'google/gemini-*' model via "
            "ask_openrouter), or trim `context`."
        ),
    )
    if too_large:
        return OracleResult(
    ...
    try:
        ...
        cap = await cli_gate.run_cli_async(
            argv, gate="agy", timeout=timeout, cwd=str(oracle_cwd())
        )
    ...
    if not answer:
        # Empty stdout with rc 0 is agy's failure mode for a DENIED tool in headless
        # mode (e.g. a read_url permission the policy auto-denied): it explains
        # itself on stderr and prints nothing, so surface that instead of a bare
        # "no answer" — the fix (an allow-rule) is in the message we'd otherwise drop.
        detail = "Gemini returned no answer"
        tail = (cap.stderr or "").strip()
        if tail:
            detail = f"{detail} — agy stderr: {tail[-600:]}"
        return OracleResult(
            "error", kind="sdk_error", text=detail, model=model,
            returncode=cap.returncode,
            telemetry=_error_telemetry(
                model, wall_duration_ms=(time.perf_counter() - started) * 1000,
                returncode=cap.returncode,
