...
def save(patch: dict) -> str | None:
    ...
    crash mid-save can never leave a half-written file that load() would silently
    turn into ``{}``. The read-modify-write is serialized across instances by an
    exclusive flock on a sidecar ``.lock`` file: without it two concurrent savers
    both read the same state, and the second's whole-file replace clobbered the
    field the first had just written (a cross-instance lost update). Under the lock
    each save sees the other's committed field, so both survive."""
    p = path()
    try:
...
def get_list(key: str, *, keep_empty: bool = False) -> list[str] | None:
    """A non-empty list-of-strings config value, or None. Accepts either a JSON
    array or a comma/space-separated string for convenience. ``keep_empty`` reads an
    explicit empty array as ``[]`` rather than None — for a list whose "set to
    nothing" must still override its env default (the operator denylist)."""
    v = load().get(key)
    if isinstance(v, list):
        out = [str(x).strip() for x in v if str(x).strip()]
        return out if out or keep_empty else None
