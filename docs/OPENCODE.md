...
To enable the `glm` and `deepseek` council oracles plus the
`ask_model(provider="glm")` and `ask_model(provider="deepseek")` single calls, put
their keys in `environment` (the deepseek key
...
`ASK_FABLE_PROJECT_ROOT` (required if you want `context(op="pack")` to be able to read
...
`ask` (Fable; `oracle="opus"` for Claude Opus 5, same OAuth session) is
the only tool that works out of the box. Every other backend is
...
|----------------|---------------|----------------|
| `ask_model(provider="minimax")` (MiniMax-M3) | `mmx` CLI | install `mmx`, then `mmx auth login` |
| `ask_model(provider="gemini")` (Gemini 3.1 Pro) | `agy` CLI | install `agy`, then sign in once |
| `ask_model(provider="codex")` (GPT-5.6 Sol) | `codex` CLI | install `codex`, then `codex login` |
| `ask_model(provider="grok")` (Grok 4.6) | `grok` CLI | install `grok`, then sign in once |
| `ask_model(provider="kimi")` (kimi-code/k3) | `kimi` CLI | install `kimi` (Kimi Code), then `kimi login` |
| `ask_model(provider="ollama")` / `ask_council(provider="ollama")` | `ollama` daemon | `ollama serve`, then `ollama signin` for `:cloud` models |
| `ask_model(provider="lmstudio")` / `ask_council(provider="lmstudio")` | LM Studio server (LAN) | Start LM Studio's server on `:1234`; optional `ASK_FABLE_LMSTUDIO_*` overrides |
| `host_status` / `unload_lms_model` | control panel / LM Studio | Optional `ASK_FABLE_CONTROL_URL` (default `http://192.0.2.10:5000`) |
| `ask_model(provider="glm")` (GLM-5.2) | Z.ai API key | `ASK_FABLE_GLM_API_KEY` in `environment` |
| `ask_model(provider="deepseek")` (deepseek-v4-pro) | DeepSeek API key | `ASK_FABLE_DEEPSEEK_API_KEY` in `environment` |
| `ask_model(provider="atlas", model=…)` / `atlas:<model-id>` | Atlas HTTP key or local Grok CLI | Set `ASK_FABLE_ATLAS_API_KEY` or `ATLASCLOUD_API_KEY` for HTTP; `xai/grok-*` uses authenticated local `grok` when available |
| `ask_model(provider="openrouter")` / `ask_council(provider="openrouter")` | OpenRouter API key | `ASK_FABLE_OPENROUTER_API_KEY` (or `OPENROUTER_API_KEY`) in `environment`; the catalog lookup needs no key |
...
opencode prefixes MCP tool names with the server name, so the 28 tools arrive as
`mcp__ask_fable__ask`, `mcp__ask_fable__ask_council`, `mcp__ask_fable__ask_model`,
`mcp__ask_fable__context`, and so on. Your agent calls them like any other
tool.
...
- **Atlas selection:** call
  `list_models({provider:"atlas", task:"debug a large Rust repository"})` when
  choosing an
  ...
  Pass an accepted `selection.model` and `selection.effort` to
  `ask_model({provider:"atlas", …})`.
...
- **Context bus:** `context(op="write", …)` a large blob once under a key, then pass
  `context_ref=<key>` on any ask tool instead of re-pasting. To pack repo files
  by path, set `ASK_FABLE_PROJECT_ROOT` and use `context(op="pack", paths=[…])`.
...
**`ask` returns an error / `ask_model(provider="glm"|"deepseek")` reports `not_configured`.**
`ask` needs the Claude Code CLI signed in (it rides the OAuth session);
`ask_model(provider="glm")` needs `ASK_FABLE_GLM_API_KEY` and
`ask_model(provider="deepseek")` needs
`ASK_FABLE_DEEPSEEK_API_KEY` in `environment`. All are reported with a `kind`
field rather than crashing.

**`ask_model(provider="gemini")` / `ask_model(provider="codex")` report `binary_missing`.** The `agy` / `codex` CLI
isn't installed or not on opencode's PATH. Install and authenticate it (see
[§5](#5-optional-backends)).

**`ask_council(provider="ollama")` returns every model as `sdk_error`.** The local `ollama`
...
> Separately from opencode's own picker, the `ask_model(provider="openrouter")` /
> `list_models(provider="openrouter")` / `ask_council(provider="openrouter")` MCP tools let the *agent*
...
The `ask_model(provider="atlas")` / `list_models(provider="atlas")` MCP tools let
the *agent* call an Atlas
